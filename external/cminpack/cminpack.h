...
/* Cmake will define cminpack_EXPCRTS on Windows when it
configures to build a shared library. If you are going to use
another build system on windows or create the visual studio
projects by hand you need to define cminpack_EXPCRTS when
building a DLL on windows.
*/
#if defined (__GNUC__)
#define CMINPACK_DECLSPEC_EXPCRT  __declspec(__dllexport__)
#define CMINPACK_DECLSPEC_IMPCRT  __declspec(__dllimport__)
#endif
#if defined (_MSC_VER) || defined (__BORLANDC__)
#define CMINPACK_DECLSPEC_EXPCRT  __declspec(dllexport)
#define CMINPACK_DECLSPEC_IMPCRT  __declspec(dllimport)
#endif
#ifdef __WATCOMC__
#define CMINPACK_DECLSPEC_EXPCRT  __export
#define CMINPACK_DECLSPEC_IMPCRT  __import
#endif
#ifdef __IBMC__
#define CMINPACK_DECLSPEC_EXPCRT  _Export
#define CMINPACK_DECLSPEC_IMPCRT  _Import
#endif

#if !defined(CMINPACK_NO_DLL) && (defined(__WIN32__) || defined(WIN32) || defined (_WIN32))
#if defined(cminpack_EXPCRTS) || defined(CMINPACK_EXPCRTS) || defined(CMINPACK_DLL_EXPCRTS)
    #define  CMINPACK_EXPCRT CMINPACK_DECLSPEC_EXPCRT
  #else
    #define  CMINPACK_EXPCRT CMINPACK_DECLSPEC_IMPCRT
  #endif /* cminpack_EXPCRTS */
#else /* defined (_WIN32) */
 #define CMINPACK_EXPCRT
#endif
...
/* the iflag parameter is input-only (with respect to the FCRTRAN */
...
int CMINPACK_EXPCRT hybrd1 ( minpack_func_nn fcn, 
...
int CMINPACK_EXPCRT hybrd ( minpack_func_nn fcn,
...
int CMINPACK_EXPCRT hybrj1 ( minpack_funcder_nn fcn, void *p, int n, double *x,
...
/* find a zero of a system of N nonlinear functions in N variables by
   a modification of the Powell hybrid method (user-supplied Jacobian,
   more general) */
int CMINPACK_EXPCRT hybrj ( minpack_funcder_nn fcn, void *p, int n, double *x,
...
int CMINPACK_EXPCRT lmdif1 ( minpack_func_mn fcn,
...
/* minimize the sum of the squares of nonlinear functions in N
   ...
   general) */
int CMINPACK_EXPCRT lmdif ( minpack_func_mn fcn,
...
int CMINPACK_EXPCRT lmder1 ( minpack_funcder_mn fcn,
...
int CMINPACK_EXPCRT lmder ( minpack_funcder_mn fcn,
	      void *p, int m, int n, double *x, double *fvec, double *fjac,
...
int CMINPACK_EXPCRT lmstr1 ( minpack_funcderstr_mn fcn, void *p, int m, int n,
...
int CMINPACK_EXPCRT lmstr (  minpack_funcderstr_mn fcn, void *p, int m,
...
void CMINPACK_EXPCRT chkder ( int m, int n, const double *x, double *fvec, double *fjac,
	       int ldfjac, double *xp, double *fvecp, int mode,
	       double *err  );

double CMINPACK_EXPCRT dpmpar ( int i );

double CMINPACK_EXPCRT enorm ( int n, const double *x );
...
/* compute a forward-difference approximation to the m by n jacobian
   ...
   variables. */
int CMINPACK_EXPCRT fdjac2(minpack_func_mn fcn,
...
int CMINPACK_EXPCRT fdjac1(minpack_func_nn fcn,
...
void CMINPACK_EXPCRT covar(int n, double *r, int ldr, 
...
*/
int CMINPACK_EXPCRT covar1(int m, int n, double fsumsq, double *r, int ldr, 
                           const int *ipvt, double tol, double *wa);
